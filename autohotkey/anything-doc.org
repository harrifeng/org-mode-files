...
* how to  write an anything-source
  ...
  now it support 7 anything-source-properties :
  + name
  + action
  + candidate
  + icon
  ...
  + anything-action-when-2-candidates
  for example:
  #+begin_src c
        my_source:=Object()
  #+end_src
...
then if only one candidate left on the listview it will execute the
    default action with the only one candidate as parameter 
 
** <anything-action-when-2-candidates> (optional)
