...
* 操作符重载
  ...
  #+END_SRC
* 面向对象 繼承 多态(动态绑定,父类引用指向子类对象)) 封装
  1. 父类 通常 应该 将 子类需要重新定义的函数 声明为 virtual
  2. 继承语法  子类:[public |private|protected] 父类
  3. 子类可以选择性的继承父类的 virtaul 函数
     #+BEGIN_SRC c++
        class Parent
        {
        public:
...
* 子类 权限的控制
  ...
  1. class Child:public Parent ,原来为public 则为public,原来为protected 则为  protected
  2. class Child:protect Parent ,原来为public 则为protected,原来为protected 则为
     protected(默认)
  3. class Child:private Parent ,parent所有成员在Child为成为private

  1. public 的,可以继承Parent继承的接口, ,private protected 不行
 #+BEGIN_SRC c++
   class Base{};
